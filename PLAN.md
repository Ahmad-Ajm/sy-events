...
- ✅ تنفيذ السلايدر في الصفحة الرئيسية (Backend + Frontend Public) وجاهز للاستخدام.
- 📝 سيتم حذف مشروع Next.js لاحقاً بعد التأكد من استقرار المنصة الجديدة (لا تبعيات عليه).

### مستجدات إضافية (14-10-2025)
- ✅ FullCalendar مدمج في واجهة التقويم (عرض شهر/أسبوع/يوم/قائمة + RTL)
- ✅ فلاتر متقدمة للأحداث (Organizer/Upcoming/MinAttendees/City/Category/Time) مفعّلة في الـ API والواجهة
- ☑ رفع ملفات متعدد جاهز في الـ Backend؛ يحتاج واجهة متعددة الملفات في Angular
- ☑ إدارة السلايدر وإعداداته (عدد العناصر + الأنواع Latest/Popular/Custom) جاهزة؛ تحتاج Seed/تغذية مبدئية

### ✅ مستجدات حاسمة (15-10-2025 - 12:30 PM)

**🎉 النظام يعمل الآن بنجاح!**

#### إصلاحات البنية التحتية:
1. **نظام الترحيلات (Migrations):**
   - ✅ إنشاء `EventManagementMigrationsDbContext` موحّد يشمل كيانات ABP + التطبيق
   - ✅ إنشاء `EventManagementMigrationsDbContextFactory` لدعم EF Core CLI
   - ✅ تكوين Npgsql كمزود قاعدة البيانات
   - ✅ ترحيلة موحدة `Init_AllModules_WithUser` (97 جدول ABP + جداول التطبيق)

2. **إصلاح DI و Seeding:**
   - ✅ تسجيل كيان `User` في DbContext
   - ✅ تكوين `User` في ModelCreatingExtensions
   - ✅ إصلاح قيود NOT NULL في Seeder (Categories: Description/Icon, Users: Phone/Profession/Interests/Reason)
   - ✅ معالجة أخطاء "Sequence contains no elements" باستخدام `FirstOrDefault()`

3. **إصلاح Swagger:**
   - ✅ حذف `CalendarController.cs` المتضارب (الاعتماد على ABP Auto API)

4. **الحالة الحالية:**
   - ✅ PostgreSQL متصل ويعمل (Port 5432)
   - ✅ DbMigrator نفّذ الترحيلات والتسييد بنجاح
   - ✅ API يعمل على `https://localhost:44388`
   - ✅ Swagger UI متاح: `https://localhost:44388/swagger`
   - ✅ بيانات أولية: 4 مدن، 3 تصنيفات، 2 مستخدمين، 2 فعاليات تجريبية

### المتطلبات العامة للواجهة العامة (Public Frontend)
- السلايدر في الصفحة الرئيسية (2-6 شرائح): أحدث/الأكثر شعبية/مخصص، قابل للتهيئة من لوحة الإدارة مع اختيار عناصر محددة وعددها.
- 3 مربعات صور أسفل السلايدر (أحدث/الأكثر شعبية/مخصصة) قابلة للتهيئة.
- الزائر يعرض الفعاليات؛ عند الضغط على "متابعة الفعالية" يتم توجيهه للتسجيل/تسجيل الدخول.
- المستخدم المسجل يرى: "التقويم" (Calendar) و"حسابي" (إدارة الحساب/الحذف/استكمال البيانات).
- التقويم بألوان: أخضر (حضر) / أحمر (تابع وتغيب) / أصفر (انقضت ولم يتابع) / أزرق (قادمة ولم يتابع) / لون آخر (قادمة ويتابع) مع جدول دلالات الألوان.
- خيار التسجيل كـ "متابع فقط".
- زر "إضافة فعالية" يرفع الدور إلى Organizer بموافقة المستخدم، ثم نموذج إضافة فعالية من 3 خطوات.
- موافقة المدير مطلوبة لكل فعالية مع خيار "الموافقة على الجميع" وتفعيل "الموافقة التلقائية على كل الفعاليات المستقبلية".

### معايير قبول مضافة
- [x] السلايدر يعمل على الصفحة الرئيسية العامة ويُدار من الإدارة (Implemented).
- [ ] المربعات الثلاث أسفل السلايدر قابلة للتهيئة من الإدارة.
- [ ] التقويم بالألوان مع جدول الدلالات.
- [ ] ترقية Viewer إلى Organizer عند إضافة فعالية (3 خطوات).
- [ ] إعدادات الموافقة (يدوي/تلقائي/جماعي) فعّالة في لوحة الإدارة.
